...
void ellipse::draw_border (const vertex& center, 
  ...
  for(int i = 0; i < 360; i++){
    float degInRad = i * DEG2RAD;
    glVertex2f((cos(degInRad)*dimension.xpos)
               +center.xpos+window::border_size
               ,(sin(degInRad)*dimension.ypos)
...
void polygon::draw_border (const vertex& center,
  ...
  for(auto i = new_vertex.begin(); i != new_vertex.end(); ++i){
    //if(i->xpos < center.xpos and i->ypos < center.ypos){
      glVertex2f(i->xpos-window::border_size
		,i->ypos-window::border_size);
      //}
      /*
    if(i->xpos > center.xposand i->ypos < center.ypos){
      glVertex2f(i->xpos+window::border_size
		 ,i->ypos-window::border_size);
    }
    if(i->xpos < center.xposand i->ypos < center.ypos){
